select 
5 mb, --目标
round(case when sum(ordernum - closenum) = 0 then 0 else sum(yqdhnum)/sum(ordernum - closenum)*100 end,2) yqbl,--逾期比例
case when (case when sum(ordernum - closenum) = 0 then 0 else sum(yqdhnum)/sum(ordernum - closenum)*100 end) = 0 then 100 
  else round(5/(case when sum(ordernum - closenum) = 0 then 0 else sum(yqdhnum)/sum(ordernum - closenum)*100 end)*100,2) end dcl, --达成率
...
/*          left outer join po_order_bb bb
  ...
  where deptname not in ('采购科','工程市场合同部','工程市场部')
