select
  ...
  case when sum(case when tt1.jhzxxhsj>tt1.jhzxzq then 1 else 0 end)/count(tt1.pk_praybill_b)*100 = 0 then 100 else
  round(5/(sum(case when tt1.jhzxxhsj>tt1.jhzxzq then 1 else 0 end)/count(tt1.pk_praybill_b)*100)*100,2) end dcl, --达成率
...
(select
    ...
    case when prb.naccumulatenum > 0 and orderb.forderstatus=3 then 1 else 0 end sfycg, 
   case when bdm.def6='~' then 999 end jhzxzq, 
...
where prb.dr = 0
  ...
  and (SUBSTR(prh.dmakedate, 0, 10)  between concat(trunc(add_months(trunc(sysdate, 'YEAR'), -12), 'yyyy'), '-12-26') and to_char(sysdate,'yyyy-MM-dd'))
  and prb.vbdef1 != '~' 
